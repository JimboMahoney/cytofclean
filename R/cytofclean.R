...
tkgrid(tklabel(tt, text = ""),tklabel(tt, text = ""),tklabel(tt, text = "Version: 0.3 beta"))
...
if (tclvalue(ret_var) != "OK") {
  ...
  ResidualMax <- NULL
  for (i in 1:length(filesToOpen)){
    ResidualMin[i] <- min(Residual_Density[[i]]$x[Residual_Density[[i]]$y>max(Residual_Density[[i]]$y)*.33])
    ResidualMax[i] <- max(Residual_Density[[i]]$x[Residual_Density[[i]]$y>max(Residual_Density[[i]]$y)*.33])
  }
