...
### Optional "strength" filter setting for user?
### Use same gate for all files?
### Show plots before saving file?
...
if (tclvalue(ret_var) != "OK") {
  ...
  for (i in 1:length(filesToOpen)){
    ...
    OffsetMin[i] <- OffsetMode[i] - 1.1 * OffsetSD[i] * 1/OffsetBias
    OffsetMax[i] <- OffsetMode[i] + 1.1 * OffsetSD[i] * OffsetBias
  }
