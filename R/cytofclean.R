...
tkgrid(tklabel(tt, text = ""),tklabel(tt, text = ""),tklabel(tt, text = "Version: 0.4 beta"))
...
if (tclvalue(ret_var) != "OK") {
  ...
  EventMax <- NULL
  for (i in 1:length(filesToOpen)){
    EventMin[i] <- min(Event_Density[[i]]$x[Event_Density[[i]]$y>max(Event_Density[[i]]$y)*.4])
    EventMax[i] <- max(Event_Density[[i]]$x[Event_Density[[i]]$y>max(Event_Density[[i]]$y)*.4])
  }
  ...
  CentreMax <- NULL
  for (i in 1:length(filesToOpen)){
    CentreMin[i] <- min(Centre_Density[[i]]$x[Centre_Density[[i]]$y > max(Centre_Density[[i]]$y)*.2])
    CentreMax[i] <- max(Centre_Density[[i]]$x[Centre_Density[[i]]$y > max(Centre_Density[[i]]$y)*.2])
  }
  ...
  OffsetMax <- NULL
  for (i in 1:length(filesToOpen)){
    OffsetMin[i] <- min(Offset_Density[[i]]$x[Offset_Density[[i]]$y>max(Offset_Density[[i]]$y)*.25])
    OffsetMax[i] <- max(Offset_Density[[i]]$x[Offset_Density[[i]]$y>max(Offset_Density[[i]]$y)*.25])
  }
  ...
  OffsetYAxis <- round((mean(OffsetMax)-mean(OffsetMin)) * 10,-1)
  ...
  ResidualMax <- NULL
  for (i in 1:length(filesToOpen)){
    ResidualMin[i] <- min(Residual_Density[[i]]$x[Residual_Density[[i]]$y>max(Residual_Density[[i]]$y)*.4])
    ResidualMax[i] <- max(Residual_Density[[i]]$x[Residual_Density[[i]]$y>max(Residual_Density[[i]]$y)*.4])
  }
  ...
  # Get Mode for Y axis
  ResidualYAxis <- round((mean(ResidualMax)-mean(ResidualMin)) * 6,-2)
  ...
  #  AxisMax[i] <- max(exprs(fcs_raw[[i]]$`Ce140Di`))
  #}

 # AxisMax <- round(mean(NotBeadMed) + 5 * mean(NotBeadSD),-2)
